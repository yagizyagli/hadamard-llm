...
class GroqModel(BaseModel):
    """
    High-performance Groq API implementation for blazing-fast inference
    using raw HTTPX async requests.
    """

def __init__(self, model_name: str = "llama-3.3-70b-versatile", temperature: float = 0.7, api_key: Optional[str] = None, **kwargs: Any):
        super().__init__(model_name, temperature, **kwargs)
        self.api_key = api_key or os.getenv("GROQ_API_KEY")
        if not self.api_key:
            raise MissingAPIKeyError("Groq API Key not found. Set GROQ_API_KEY env var or pass it explicitly.")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code != 200:
                    raise ModelProviderError("Groq", response.status_code, response.text)
                return response.json()["choices"][0]["message"]["content"]
            except (httpx.HTTPError, KeyError, IndexError) as exc:
                raise ModelProviderError("Groq", 500, f"Request failed: {str(exc)}")
        ...
        async with httpx.AsyncClient() as client:
            try:
                async with client.stream("POST", self.base_url, json=payload, headers=headers, timeout=60.0) as response:
                    if response.status_code != 200:
                        raise ModelProviderError("Groq", response.status_code, await response.aread())
                ...
                raise ModelProviderError("Groq", 500, f"Streaming failed: {str(exc)}")
